...
RUN set -ex \
    ...
    && make install \
    && cp objs/ngx_http_small_light_module.so /etc/ngx_small_light/ \
...
# RUN apk add https://extras.getpagespeed.com/release-latest.rpm
# RUN apk add install nginx-module-small-light
# RUN cp objs/ngx_http_small_light_module.so /etc/ngx_small_light/
